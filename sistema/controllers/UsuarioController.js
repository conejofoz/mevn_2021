...
export default{
    ...
    list: async(req, res, next) => {
        try {
            ...
            //const reg = await models.Usuario.find({'nombre': new RegExp(valor, 'i')},{createdAt:0}).sort({'nombre':1})
            const reg = await models.Usuario.find({$or:[{'nombre': new RegExp(valor, 'i')},{'email': new RegExp(valor, 'i')}]},{createdAt:0}).sort({'nombre':1})
            res.status(200).json(reg)
        } catch (e) {
            res.status(500).send({
                message:'Ocurrió un error'
    ...
    update: async(req, res, next) => {
        try {
            ...
            let passwordNovo = req.body.password
            const usuarioAntigo = await models.Usuario.findOne({_id:req.body._id})
            if(passwordNovo!=usuarioAntigo.password){
                req.body.password = await bcrypt.hash(req.body.password, 10)
            }
