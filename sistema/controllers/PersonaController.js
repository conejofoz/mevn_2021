...
export default{
    ...
    list: async(req, res, next) => {
        try {
            ...
            //const reg = await models.Persona.find({'nombre': new RegExp(valor, 'i')},{createdAt:0}).sort({'nombre':1})
            const reg = await models.Persona.find({$or:[{'nombre': new RegExp(valor, 'i')},{'email': new RegExp(valor, 'i')}]},{createdAt:0}).sort({'nombre':1})
            res.status(200).json(reg)
        } catch (e) {
            res.status(500).send({
                message:'Ocurrió un error'
    ...
    listClientes: async(req, res, next) => {
        try {
            let valor = req.query.valor
            //const reg = await models.Persona.find({'nombre': new RegExp(valor, 'i')},{createdAt:0}).sort({'nombre':1})
            const reg = await models.Persona.find({$or:[{'nombre': new RegExp(valor, 'i')},{'email': new RegExp(valor, 'i')}], 'tipo_persona': 'Cliente'},{createdAt:0}).sort({'nombre':1})
            res.status(200).json(reg)
        } catch (e) {
            res.status(500).send({
                message:'Ocurrió un error'
    ...
    listProveedores: async(req, res, next) => {
        try {
            let valor = req.query.valor
            //const reg = await models.Persona.find({'nombre': new RegExp(valor, 'i')},{createdAt:0}).sort({'nombre':1})
            const reg = await models.Persona.find({$or:[{'nombre': new RegExp(valor, 'i')},{'email': new RegExp(valor, 'i')}], 'tipo_persona': 'Proveedor'},{createdAt:0}).sort({'nombre':1})
            res.status(200).json(reg)
        } catch (e) {
            res.status(500).send({
                message:'Ocurrió un error'
